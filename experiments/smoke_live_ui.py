...
def run(url, output, executable=None):
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_role("button", name="1 Прочитать старый runbook", exact=False).click()
        expect(page.locator("#answer")).to_contain_text("payments-eu-west")
        page.get_by_role("button", name="2 Найти запись о миграции", exact=False).click()
        expect(page.locator("#answer")).to_contain_text("Стоп: не перезапускайте")
        page.locator(".fact-node").filter(has_text="payments-eu-central").click()
        expect(page.locator("#inspector")).to_contain_text(
            "scenario://checkout/migration-v1"
        )
        page.screenshot(path=str(output / "live-conflict.png"), full_page=True)
        page.get_by_role("button", name="3 Подтвердить миграцию", exact=False).click()
        expect(page.locator("#answer")).to_contain_text("Безопасный target")
        ...
        expect(page.locator("#answer .answer-value")).to_have_text("payments-eu-west")
        page.reload()
        expect(page.locator("#answer .answer-value")).to_have_text(
            "payments-eu-central"
        )
        ...
        expect(page.locator("#answer .answer-value")).to_have_text("payments-test")
        page.locator(".fact-node").filter(has_text="payments-test").click()
